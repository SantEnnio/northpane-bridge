...
public struct ConnectionProfile: Identifiable, Equatable, Codable, Sendable {
    ...
    public init(id: UUID = UUID(), kind: TransportKind, endpoint: String? = nil, priority: Int = 0, credentialReference: CredentialReference? = nil, expectedHostFingerprint: String? = nil, expectedTransportFingerprint: String? = nil, lastReachedTailnet: String? = nil) {
        self.id = id; self.kind = kind; self.endpoint = endpoint; self.priority = priority; self.credentialReference = credentialReference; self.expectedHostFingerprint = expectedHostFingerprint; self.expectedTransportFingerprint = expectedTransportFingerprint; self.lastReachedTailnet = lastReachedTailnet
    }
}
